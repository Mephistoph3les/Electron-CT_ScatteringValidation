...
if __name__ == '__main__':
    ...
    for i in range(len(thicknesses_alu)):
        ...
        directory = 'Simulation_data/{}_{}_{}_events_60MeV'.format(material, thick, events)
        ...
        for j in range(len(EM_modules_list)):
            ...
            if module == "EM_Opt1":
                command = '../../allpix-squared/install/bin/allpix -c %s -j%s -o output_directory="%s" -o detectors_file="%s" -o number_of_events="%s" -o physics_list="FTFP_BERT%s" -o msc_energy_limit="20MeV" -o source_energy="60MeV"' % (config, cores, output_directory, geometry_file, events, module_mark)
            else:
                command = '../../allpix-squared/install/bin/allpix -c %s -j%s -o output_directory="%s" -o detectors_file="%s" -o number_of_events="%s" -o physics_list="FTFP_BERT%s" -o source_energy="60MeV"' % (config, cores, output_directory, geometry_file, events, module_mark)
            os.system(command)
            end_time = time.time()
    ...
    time_file = 'timing_data/computing_time_6000um_high_statistics_Wentzel_Urban_Goudsmit_60_MeV.csv'
