...
if __name__ == '__main__':
    ...
    for i in range(len(thicknesses_alu)):
        ...
        for j in range(len(EM_modules_list)):
            ...
            if module == "EM_Opt1":
            	...
            	command = '../../allpix-squared/install/bin/allpix -c %s -j%s -o output_directory="%s" -o detectors_file="%s" -o number_of_events="%s" -o physics_list="FTFP_BERT%s" -o msc_energy_limit=200MeV' % (config, cores, output_directory, geometry_file, events, module_mark)
            os.system(command)
            end_time = time.time()
    ...
    time_file = 'timing_data/computing_time_%s_1000um_%s.csv' % (material, events)
